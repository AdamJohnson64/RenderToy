...
namespace RenderToy.BoundingVolumeHierarchy
{
    ...
    /// </summary>
    public static partial class OctreeREF
    {
        ...
        static MeshBVH CreateLooseOctreeChild(Triangle3D[] triangles, int level, Bound3D box)
        {
            ...
            return Create(contained_triangles, level - 1);
        }
        static Triangle3D[] CreateTriangleList(Triangle3D[] triangles, Bound3D box)
        {
            if (triangles.Length < 32)
            {
                return triangles.Where(t => CommonBVH.ShapeIntersects(box, new Triangle3D(t.P0, t.P1, t.P2))).ToArray();
            }
            else
            {
                return triangles.AsParallel().Where(t => CommonBVH.ShapeIntersects(box, new Triangle3D(t.P0, t.P1, t.P2))).ToArray();
            }
        }
