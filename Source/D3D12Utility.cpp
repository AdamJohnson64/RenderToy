...
namespace Arcturus
{
    ...
    ID3D12Resource1* D3D12CreateBuffer(ID3D12Device* device, D3D12_RESOURCE_FLAGS flags, D3D12_RESOURCE_STATES state, uint32_t bufferSize, uint32_t dataSize, const void* data, ID3D12CommandQueue* commandQueue, ID3D12CommandAllocator *commandAllocator)
    {
        ...
        AutoRelease<ID3D12Resource1> upload;
        D3D12_HEAP_PROPERTIES descHeapProperties = {};
        ...
        descResource.SampleDesc.Count = 1;
        descResource.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
        TRYD3D(device->CreateCommittedResource(&descHeapProperties, D3D12_HEAP_FLAG_NONE, &descResource, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, __uuidof(ID3D12Resource1), (void**)&upload));
        ...
        TRYD3D(device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, commandAllocator, nullptr, __uuidof(ID3D12GraphicsCommandList5), (void**)&uploadCommandList));
