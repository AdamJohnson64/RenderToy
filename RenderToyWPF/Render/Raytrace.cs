...
namespace RenderToy
{
    static class Raytrace
    {
            ...
            for (int y = 0; y < buffer_height; ++y)
            {
                for (int x = 0; x < buffer_width; ++x)
                {
                    ...
                    if (found_lambda != double.PositiveInfinity)
                    {
                        ...
                        double shadow_multiplier = RayShadowTest(objects, shadow_origin, light_vector) ? 0.5 : 1.0;
        ...
        static bool RayShadowTest(RaytraceObject[] objects, Point3D shadow_origin, Vector3D light_vector)
        {
            foreach (var test in objects)
            {
