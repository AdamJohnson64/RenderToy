...
namespace Arcturus
{
    IRenderTarget_D3D12::IRenderTarget_D3D12(IDevice3D_D3D12* owner, const RenderTargetDeclaration& declaration) : m_owner(owner)
    {
        ...
        descClear.DepthStencil.Depth = 1.0f;
        {
            ...
            TRYD3D(owner->m_device->CreateCommittedResource1(&descHeapProperties, D3D12_HEAP_FLAG_ALLOW_ALL_BUFFERS_AND_TEXTURES | D3D12_HEAP_FLAG_SHARED, &descResource, D3D12_RESOURCE_STATE_COMMON, &descClear, nullptr, __uuidof(ID3D12Resource1), (void**)&m_resource));
        }
    ...
    {
        TRYD3D(m_owner->m_device->OpenSharedHandle(handle, __uuidof(ID3D12Resource1), (void**)&m_resource));
    }
}