...
static void RenderImageCPU(const void* scene, const void* inverse_mvp, void* bitmap_ptr, int render_width, int render_height, int bitmap_stride) {
	for (int y = 0; y < render_height; ++y) {
		for (int x = 0; x < render_width; ++x) {
			RaytraceCLI::PixelSetARGB<FLOAT> setpixel(bitmap_ptr, render_width, render_height, bitmap_stride, x, y);
			RaytraceCLI::ComputePixel<FLOAT, RENDERMODE>(*(Scene<FLOAT>*)scene, *(Matrix44<FLOAT>*)inverse_mvp, setpixel, 1, 1);
		}
	}
}
