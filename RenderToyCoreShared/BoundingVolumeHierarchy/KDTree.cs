...
{
    public static partial class KDTree
    {
        public static MeshBVH Create(Triangle3D[] triangles)
        {
            return Create(triangles, CommonBVH.MAXIMUM_BVH_DEPTH);
        }
        public static MeshBVH Create(Triangle3D[] triangles, int level)
        {
            var bound = CommonBVH.ComputeBounds(triangles);
            ...
            var node_back = Create(triangles_back.ToArray(), level - 1);
            var node_frnt = Create(triangles_frnt.ToArray(), level - 1);
