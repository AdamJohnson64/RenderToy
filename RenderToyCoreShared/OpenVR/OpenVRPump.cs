        ...
        public static Action CreateRenderer(SparseScene scene)
        {
            ...
            return () =>
            {
                ...
                Task<ID3D11CommandList> do_left = Task.Factory.StartNew(() =>
                {
                    ...
                    var transformProjection = openvr.GetProjectionMatrix(Eye.Left, 0.1f, 2000.0f);
                    ...
                    constants["transformViewProjection"] = transformView * transformProjection;
                ...
                Task<ID3D11CommandList> do_right = Task.Factory.StartNew(() =>
                {
                    ...
                    var transformCamera = MathHelp.Invert(transformView);
                    var transformProjection = openvr.GetProjectionMatrix(Eye.Right, 0.1f, 2000.0f);
                    var constants = new Dictionary<string, object>();
